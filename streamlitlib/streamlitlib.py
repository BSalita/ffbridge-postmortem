...
def ShowDataFrameTable(table_df,key=None,output_method='aggrid',color_column=None,ngroup_name=None,round=2,tooltips=None):
    ...
    if output_method == 'table':
        ...
        AgGrid(
            ...
            # height calc is a bit arbitrary. add 50 for header. add 40 per row for text. add 10 per row for line separators.  add 22 in case there's a horizontal scrollbar.
            height=50+(30+10)*min(4,len(table_df))+22,
            #width='100%',
            #reload_data=True
            key=key
            )
