...
def build_classifier_mlp(input_dim: int, num_classes: int, layers: List[int], dropout: float = 0.2, cat_dims: Optional[List[Tuple[int, int]]] = None) -> torch.nn.Module:
    """Builds an MLP classifier, using MLPClassification for consistent architecture."""
    return MLPClassification(dim_in=input_dim, cat_dims_local=cat_dims, widths=layers, dropout=dropout, out_classes=num_classes)


class MLPClassification(torch.nn.Module):
    """Multi-Layer Perceptron for classification with optional embeddings for categorical features.

    This class is used for inference to reconstruct the model.
    """
    def __init__(self, dim_in: int, cat_dims_local: Optional[List[Tuple[int, int]]] = None, widths: List[int] = [1024, 512, 256, 128], dropout: float = 0.1, out_classes: int = 1):
        super().__init__()
        ...
        total_input_dim = dim_in + embedding_output_dim
        ...
        blocks.append(torch.nn.Linear(prev, out_classes))  # Final layer outputs num_classes
        self.net = torch.nn.Sequential(*blocks)
    ...
    def forward(self, x_cont, x_cat=None):
        ...
        elif x_cat is not None: # Direct concatenation if no embeddings
            x = torch.cat([x_cont, x_cat.float()], 1) # Ensure categorical is float for concat
        else:
            x = x_cont

        return self.net(x) # No squeeze(-1) for classification
    ...
    for col in tqdm(numeric_cols, desc="scaling-features", leave=False):
        try:
            ...
            if dt in (pl.Float32, pl.Float64):
                # Float types - direct conversion for scaling computation
                numeric_series = series.cast(pl.Float64)
            elif dt in (pl.Int8, pl.Int16, pl.Int32, pl.Int64, pl.UInt8, pl.UInt16, pl.UInt32, pl.UInt64):
                # Integer types - convert to float for scaling computation
                numeric_series = series.cast(pl.Float64)
            elif dt == pl.Boolean:
                # Boolean: convert to 0/1 for scaling computation
                ...
                raise ValueError(f"Column '{col}' has unsupported dtype '{dt}' for numerical features. "
                               f"Expected: Float32/Float64/Int*/UInt*/Boolean/Date/Datetime")
...
def build_continuous_tensor(
    ...
    # STRICT: Allow any numeric types, convert to float32 for PyTorch
    processed_cols = []
    for col in numerical_feature_cols:
        col_series = df.select(col).to_series()
        dtype_str = str(col_series.dtype)
        
        # Allow any numeric type - convert to numpy then to float32
        if dtype_str in ['Float32', 'Float64']:
            col_data = col_series.to_numpy().astype(np.float32)
            processed_cols.append(col_data)
        elif dtype_str in ['Int8', 'Int16', 'Int32', 'Int64', 'UInt8', 'UInt16', 'UInt32', 'UInt64']:
            col_data = col_series.to_numpy().astype(np.float32)
            processed_cols.append(col_data)
        elif dtype_str == 'Boolean':
            # Boolean to 0/1 then float32
            col_data = col_series.to_numpy().astype(np.float32)
            processed_cols.append(col_data)
        elif dtype_str in ['Date', 'Datetime']:
            # Date/Datetime to timestamp then float32
            if dtype_str == 'Date':
                # Convert to microseconds then to seconds (divide by 1e6)
                timestamp_data = (df.select(pl.col(col).dt.timestamp('us')).to_numpy().flatten() / 1e6).astype(np.float32)
            else:  # Datetime
                # Convert to microseconds then to seconds (divide by 1e6)
                timestamp_data = (df.select(pl.col(col).dt.timestamp('us')).to_numpy().flatten() / 1e6).astype(np.float32)
            processed_cols.append(timestamp_data)
        else:
            raise ValueError(f"Column '{col}' has unsupported dtype '{dtype_str}' for numerical features. "
                           f"Expected: Float32/Float64/Int*/UInt*/Boolean/Date/Datetime")
...
def build_categorical_tensor(
    ...
    for col in categorical_feature_cols:
        ...
        # Handle String columns - keep nulls as actual nulls
        if dtype_str in ['String', 'Utf8']:
            # Keep nulls as nulls for proper mapping
            pass  # col_data already has nulls as nulls
        
        # Check for unknown categories before mapping
        unique_values_raw = col_data.unique().to_list()
        unique_values = set(unique_values_raw)  # Keep nulls as nulls, don't convert to string
        ...
        # DEBUG: Show categorical mapping process
        if len(cat_arrays) == 0:  # Only debug first categorical column
            print(f"🔍 CATEGORICAL MAPPING DEBUG for column '{col}':")
            print(f"   Column dtype: {dtype_str}")
            print(f"   First 5 values: {col_data.head(5).to_list()}")
            print(f"   Category mapping: {mapping}")
            print(f"   Unique values in data: {sorted(col_data.unique().to_list())}")
        
        # Map categories to indices (no default since we've verified all are known)
        # Handle None values specially since replace_strict may not handle None keys correctly
        if None in mapping:
            # For columns that can have None values (like target columns), handle them explicitly
            mapped = (
                col_data
            ...
            # Standard mapping for feature columns without nulls
            mapped = (
                col_data
                .replace_strict(mapping, return_dtype=pl.Int64)
                .to_numpy()
        ...
        if len(cat_arrays) == 0:  # Only debug first categorical column
            print(f"   First 5 mapped indices: {mapped[:5].tolist()}")
            print(f"   Unique mapped indices: {sorted(set(mapped.tolist()))}")
...
def build_model_from_schema(schema: Dict[str, Any], input_dim: int, device: torch.device) -> torch.nn.Module:
    ...
    model = MLPRegression(input_dim=input_dim, cat_dims=cat_dims, layer_sizes=mlp_layers, dropout=dropout)
...
def generate_and_save_schema_core(
    ...
    # STRICT: Allow any numeric types - consistency between training and inference is key
    numerical_feature_cols = feature_float_cols + feature_int_cols + bool_cols + date_cols
    
    # STRICT: Only String/Categorical allowed for categorical features
    valid_categorical_cols = [c for c in (string_cols + categorical_cols) if df[c].dtype in (pl.String, pl.Utf8, pl.Categorical)]
    invalid_categorical = [c for c in (string_cols + categorical_cols) if df[c].dtype not in (pl.String, pl.Utf8, pl.Categorical)]
    
    if invalid_categorical:
        raise ValueError(f"Categorical features must be exactly String/Utf8/Categorical dtype. Found invalid columns: {invalid_categorical}. "
                        f"Fix the source data to have proper categorical dtypes before training.")
    
    categorical_feature_cols = valid_categorical_cols
    ...
    # Process all categorical columns (features + target if categorical)
    all_categorical_cols = categorical_feature_cols.copy()
    target_dtype = dtypes[y_name]
    if _is_categorical_dtype(target_dtype) or _is_string_dtype(target_dtype):
        all_categorical_cols.append(y_name)
    ...
    for col in all_categorical_cols:
        if col in schema_map:
            col_data = df[col]
            dt = dtypes[col]
            # Handle both String and Categorical columns - include null handling
            if _is_string_dtype(dt):
                # For String columns, keep nulls as actual nulls
                uniques = col_data.unique()
            else:
                # For Categorical columns, keep nulls as actual nulls
                uniques = col_data.unique()
            
            values = uniques.to_list() if hasattr(uniques, 'to_list') else list(uniques)
            
            # For target columns, preserve nulls if they exist in the data
            # For feature columns, filter out nulls since they should be handled separately
            has_nulls = None in values
            if col == y_name and has_nulls:
                # Target column with nulls - keep them (e.g., Declarer_Direction can be null for PASS)
                non_null_values = [v for v in values if v is not None]
                # Sort non-null values for deterministic ordering
                non_null_values = sorted(non_null_values, key=lambda x: str(x))
                # Put null first in the mapping (keep as actual None, not string 'null')
                mapping = {None: 0}
                for i, v in enumerate(non_null_values):
                    mapping[v] = i + 1
                values = [None] + non_null_values
            else:
                # Feature column or target without nulls - filter out nulls
            ...
            cat_feature_info[col] = len(values)  # cardinality includes null if present

    # Infer model type from y dtype
    target_dtype = dtypes[y_name]
    inferred_model_type = 'regression' if _is_float_dtype(target_dtype) else 'classification'

    # Create feature dtype mapping for all features used in training
    feature_dtypes = {}
    for col in feature_column_list:
        if col in dtypes:
            # Convert Polars dtype to string representation for JSON serialization
            dtype_str = str(dtypes[col])
            feature_dtypes[col] = dtype_str
    
    # Create exact dtype mapping for strict validation (includes target)
    column_dtypes = {}
    for col in feature_column_list + [y_name]:
        if col in dtypes:
            column_dtypes[col] = str(dtypes[col])
    ...
    schema = {
        ...
        # NEW: Feature dtype mapping for training features
        "feature_dtypes": feature_dtypes,
        # EXACT DTYPE VALIDATION: Store exact Polars dtypes for strict validation
        "column_dtypes": column_dtypes,
    }
    
    # For classification models, add class_to_idx mapping from target column
    if inferred_model_type == 'classification' and y_name in category_mappings:
        target_mapping = category_mappings[y_name]
        schema["class_to_idx"] = target_mapping
        schema["classes"] = list(target_mapping.keys())  # Also add classes list for compatibility
    ...
    if verbose:
        ...
        print(f"  Feature dtypes recorded: {len(feature_dtypes)} features with their original dtypes")
...
def generate_and_save_schema_classification(
    ...
    # For classification targets, preserve null values if they exist (e.g., PASS contracts)
    # Sort non-null values for deterministic ordering
    labels_clean = [c for c in labels_list if c is not None]
    has_nulls = None in labels_list
    
    if has_nulls:
        # Include null as a valid class (e.g., for PASS contracts in Declarer_Direction)
        non_null_classes = sorted(set(labels_clean), key=lambda x: str(x))
        # Put null first in the class list for consistency with schema_core
        uniq_classes: List[Any] = [None] + non_null_classes
        class_to_idx: Dict[Any, int] = {None: 0}
        for i, c in enumerate(non_null_classes):
            class_to_idx[c] = i + 1
    else:
        # No nulls - standard processing
...
def _train_model_core(
    ...
    model = MLPRegression(input_dim=input_dim, cat_dims=cat_dims, layer_sizes=layers, dropout=dropout).to(device_t)
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in train_loader:
            ...
            if use_amp and device_t.type == 'cuda':
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=100.0)  # Increased max_norm
                scaler.step(opt)
                scaler.update()
            else:
                loss.backward()
...
def train_model_from_shards(
    ...
    if model_type == 'regression':
        ...
        classification_lr = 1e-5 if lr == 2e-3 else lr  # Use default or user override (reduced LR for stability)
...
def train_classification_from_iterator(
    ...
    input_dim: int,
    num_classes: int,
    cat_dims: Optional[List[Tuple[int, int]]],
    epochs: int = 3,
    device: Optional[str] = 'cuda',
    lr: float = 1e-3,
    layers: Optional[List[int]] = None,
    ...
    model = build_classifier_mlp(input_dim, num_classes, model_layers, dropout, cat_dims=cat_dims).to(device_t)
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    loss_fn = torch.nn.CrossEntropyLoss()
    ...
    for epoch in range(epochs):
        total_loss, total_count, correct = 0.0, 0, 0
        for xb_cont, xb_cat, yb in train_iterator_fn():
            xb_cont = xb_cont.to(device_t)
            xb_cat = xb_cat.to(device_t) if xb_cat is not None else None
            yb = yb.to(device_t)
            opt.zero_grad(set_to_none=True)
            with autocast_fn():
                logits = model(xb_cont, xb_cat)
    ...
    # After training loop, load the best model state if available
    if best_model_state_dict is not None:
        model.load_state_dict(best_model_state_dict)
        if verbose:
            print(f"✅ Loaded best model state with validation accuracy: {best_val_acc:.4f}")
    ...
    def _iter(files: List[pathlib.Path]):
        for f in files:
            ...
            if y.dtype != torch.long:
                y = y.to(torch.long)
            for i in range(0, X_cont.shape[0], bs):
                xb_cont = X_cont[i:i+bs]
                yb = y[i:i+bs]
                xb_cat = X_cat[i:i+bs] if X_cat is not None and X_cat.numel() > 0 else None
                yield xb_cont, xb_cat, yb
...
def predict_regression_model(saved_models_path: pathlib.Path, model_name: str, df: pl.DataFrame, device: str = 'cpu', max_samples: Optional[int] = None) -> pl.DataFrame:
    ...
    if missing_cols:
        # Get column type information from schema
        numerical_cols = set(schema.get('numerical_feature_cols', []))
        categorical_cols = set(schema.get('categorical_feature_cols', []))
        
        missing_info = []
        for col in sorted(missing_cols):
            if col in numerical_cols:
                missing_info.append(f"{col} (numerical)")
            elif col in categorical_cols:
                missing_info.append(f"{col} (categorical)")
            else:
                missing_info.append(f"{col} (unknown type)")
        
            raise ValueError(f"Required feature columns missing from DataFrame: {missing_info}")
    
    # Check EXACT dtype compatibility between DataFrame and schema
    column_dtypes = schema.get('column_dtypes', {})
    if not column_dtypes:
        raise ValueError("Schema missing 'column_dtypes' field. All schemas must have exact dtype information.")
    
    dtype_mismatches = []
    for col in df.columns:
        if col in column_dtypes:
            expected_dtype = column_dtypes[col]
            actual_dtype = str(df[col].dtype)
            
            # STRICT: Require exact dtype match, no conversions allowed
            if actual_dtype != expected_dtype:
                dtype_mismatches.append(f"{col}: expected {expected_dtype}, got {actual_dtype}")
    
    if dtype_mismatches:
        raise ValueError(f"DataFrame column dtypes don't match schema exactly (no conversions allowed): {dtype_mismatches}")
    ...
    model = MLPRegression(input_dim=int(Xc.shape[1]), cat_dims=cat_dims, layer_sizes=mlp_layers, dropout=dropout)
    ...
    if all_zero:
        ...
        print(f"DEBUG: ✅ Model parameters have non-zero values")
            ...
            with torch.amp.autocast('cuda', enabled=(device_t.type == 'cuda')):
                ...
                if batch_idx == 0:
                    ...
                    if unique_raw == 1:
                        ...
                        print(f"  ✅ Model outputs {unique_raw} unique values")
                
                # Handle y_range: scale model output to [0,1] instead of hard clamping
                if y_range is not None and batch_idx == 0:
                    # Check if model outputs are outside y_range
                    raw_min, raw_max = raw_output.min().item(), raw_output.max().item()
                    if raw_min < y_range[0] or raw_max > y_range[1]:
                        print(f"DEBUG: Model outputs [{raw_min:.3f}, {raw_max:.3f}] outside y_range {y_range}")
                        print(f"DEBUG: Applying sigmoid scaling instead of hard clamping")
                        
                # Apply sigmoid to map any range to (0,1), then scale to y_range if needed
                yb = torch.sigmoid(raw_output)
                
                if batch_idx == 0:
                    sigmoid_vals = yb.cpu().tolist()
                    print(f"DEBUG: After sigmoid scaling:")
                    print(f"  Min: {min(sigmoid_vals):.6f}, Max: {max(sigmoid_vals):.6f}")
                    print(f"  Sample: {[f'{x:.6f}' for x in sigmoid_vals[:5]]}")
                    
                # Scale to y_range if specified
                if y_range is not None:
                    y_min, y_max = y_range
...
def predict_classification_model(
    ...
    # Check for missing required feature columns
    schema_features = schema.get('feature_column_list', [])
    if schema_features:  # Only check if schema has feature list
        missing_cols = set(schema_features) - set(df.columns)
        if missing_cols:
            # Get column type information from schema
            numerical_cols = set(schema.get('numerical_feature_cols', []))
    ...
    # DEBUG: Show tensor information
    print(f"🔍 TENSOR DEBUG:")
    print(f"   Xc (numerical) shape: {Xc.shape}, dtype: {Xc.dtype}")
    print(f"   Xk (categorical) shape: {Xk.shape if Xk is not None else 'None'}, dtype: {Xk.dtype if Xk is not None else 'None'}")
    print(f"   Uses embeddings: {uses_embeddings}")
    print(f"   Model expects {num_classes} classes")
    
    # DEBUG: Show first few samples of input data
    print(f"🔍 INPUT DATA DEBUG (first 3 samples):")
    if Xc.shape[0] >= 3:
        for i in range(3):
            print(f"   Sample {i} - Xc: {Xc[i][:5].tolist()}... (showing first 5 numerical features)")
            if Xk is not None and Xk.shape[0] >= 3:
                print(f"   Sample {i} - Xk: {Xk[i].tolist()} (categorical indices)")
    
    # DEBUG: Check if all samples are identical
    if Xc.shape[0] > 1:
        identical_numerical = torch.allclose(Xc[0], Xc[1], atol=1e-6)
        print(f"   First two numerical samples identical: {identical_numerical}")
        if Xk is not None and Xk.shape[0] > 1:
            identical_categorical = torch.equal(Xk[0], Xk[1])
            print(f"   First two categorical samples identical: {identical_categorical}")
    ...
    with torch.no_grad():
        for i in range(0, Xc.shape[0], bs):
            ...
            batch_logits_list = logits.cpu().tolist()
            print(f"🔍 MODEL OUTPUT DEBUG (first {min(5, len(batch_pred_list))} samples):")
            for j in range(min(5, len(batch_pred_list))):
                pred_idx = batch_pred_list[j]
                probs = batch_probs_list[j]
                raw_logits = batch_logits_list[j]
                top_5_probs = sorted(enumerate(probs), key=lambda x: x[1], reverse=True)[:5]
                print(f"   Sample {len(pred_codes)-len(batch_pred_list)+j}:")
                print(f"     Raw logits: {[f'{x:.4f}' for x in raw_logits]}")
                print(f"     Predicted idx: {pred_idx}")
                print(f"     Top 5 probs: {[(idx, f'{prob:.4f}') for idx, prob in top_5_probs]}")

            # Check if all predictions in this batch are identical
            if len(set(batch_pred_list)) == 1:
                print(f"   ⚠️ WARNING: All {len(batch_pred_list)} predictions in this batch are identical (idx={batch_pred_list[0]})")
            else:
                print(f"   ✅ Good: {len(set(batch_pred_list))} different predictions in this batch")
...
def display_feature_importances_regression(
    ...
    emb_dims = {col: (min(50, int(cat_feature_info.get(col, 0)) // 2) if int(cat_feature_info.get(col, 0)) > 0 else 0)
                for col in categorical_feature_cols}
    # Calculate expected input dim same way as prediction (excluding target column)
    ...
    if verbose:
        ...
        if in_features != expected_input_dim:
            print(f"⚠️ Mismatch: schema features={expected_input_dim}, model expects={in_features}")
            print(f"   Note: Schema includes {len(numerical_feature_cols)} numerical + {sum(emb_dims.values())} embedding dims")
            print(f"   This may indicate a feature count mismatch between training and inference")
...
def _get_classification_mappings_from_schema(
    saved_models_path: pathlib.Path,
    model_name: str,
) -> Tuple[Optional[List[Any]], Optional[Dict[int, Any]]]:
    """Helper to load schema and extract classification class mappings."""
    schema_path = resolve_schema_path(saved_models_path, model_name)
    schema = json.load(open(schema_path))
    
    classes_from_schema = schema.get('classes')
    class_to_idx_from_schema = schema.get('class_to_idx')
    idx_to_class_from_schema = {v: k for k, v in class_to_idx_from_schema.items()} if class_to_idx_from_schema else None
    
    return classes_from_schema, idx_to_class_from_schema
